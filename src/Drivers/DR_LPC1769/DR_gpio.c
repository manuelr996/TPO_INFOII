...
void SetMODE ( uint8_t puerto , uint8_t bit , uint8_t modo )
{
	...
	*( p + puerto * 2 + bit / 16 ) = *( p + puerto * 2 + bit / 16 ) & ~( 0x3 << (2 * (bit % 16)));
	*( p + puerto * 2 + bit / 16 ) = *( p + puerto * 2 + bit / 16 ) | ( modo << (2 * (bit % 16)));

}

void SetMODEOD ( uint8_t puerto , uint8_t bit , uint8_t modo )
{
	__RW uint32_t *p = ( __RW uint32_t * ) 0x4002C068;

	if( modo )
		*( p + puerto * 0x04) |= 0x01 << bit;
	else
		*( p + puerto * 0x04) &= ~( 0x01 << bit );
}
